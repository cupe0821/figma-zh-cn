/* Figma zh-CN translation overlay 2026-07-17 */
(()=>{
'use strict';
const map={};
Object.assign(map,{
  "Clouds":"云朵","Concentric patterns":"同心图案","Caustics":"水体焦散","Gradient map":"渐变映射","Pattern grids":"图案网格","Bloom":"泛光","Mesh gradient":"网格渐变","Fractal noise":"分形噪声","Chromatic metal":"彩色金属","Channel mixer":"通道混合","Moiré":"莫尔纹","Gooey merge":"黏液融合","Glowing waves":"发光波浪","Bokeh blur":"散景模糊","Filter presets":"滤镜预设","Nebula":"星云","Color adjust":"色彩调整","Outline":"轮廓","Colored edges":"彩色边缘","Lens distortion":"镜头畸变","Pattern refraction":"图案折射","Slice shift":"切片移位",
  "Position X":"横向位置","Position Y":"纵向位置","Center":"中心","Center X":"中心横坐标","Center Y":"中心纵坐标","Origin":"原点","Origin X":"原点横坐标","Origin Y":"原点纵坐标","Width":"宽度","Height":"高度","Resolution":"分辨率","Size":"大小","Count":"数量","Rows":"行数","Columns":"列数","Cell size":"单元格大小","Line width":"线宽","Thickness":"厚度","Color 1":"颜色 1","Color 2":"颜色 2","Color 3":"颜色 3","Color 4":"颜色 4","Primary color":"主色","Secondary color":"辅助色","Highlight color":"高光颜色","Shadow color":"阴影颜色","Midtones":"中间调","Tint":"色调","Temperature":"色温","Vibrance":"自然饱和度","Lightness":"明度","Black point":"黑场","White point":"白场",
  "Square":"方形","Lines":"线条","Blur radius":"模糊半径","Focus":"焦点","Focus distance":"焦距","Aperture":"光圈","Bokeh size":"散景大小","Samples":"采样数","Refraction amount":"折射量","Refractive index":"折射率","Chromatic dispersion":"色散","Wave count":"波浪数量","Wave speed":"波浪速度","Wave height":"波浪高度","Wavelength":"波长","Turbulence":"湍流","Flow":"流动","Viscosity":"黏度","Gravity":"重力",
  "Noise":"噪声","Noise amount":"噪声量","Noise strength":"噪声强度","Noise type":"噪声类型","Noise detail":"噪声细节","Noise seed":"噪声种子","Noise frequency":"噪声频率","Noise amplitude":"噪声振幅","Noise octaves":"噪声倍频程","Grain":"颗粒","Grain size":"颗粒大小","Granularity":"颗粒度","Randomness":"随机度","Jitter":"抖动量","Scatter":"散布","Complexity":"复杂度","Edge width":"边缘宽度","Edge color":"边缘颜色","Edge threshold":"边缘阈值","Edge intensity":"边缘强度","Slice count":"切片数量","Slice size":"切片大小","Shift":"移位","Horizontal shift":"横向移位","Vertical shift":"纵向移位",
  "Pattern scale":"图案缩放","Pattern rotation":"图案旋转","Pattern offset":"图案偏移","Pattern opacity":"图案不透明度","Grid size":"网格大小","Grid spacing":"网格间距","Grid line width":"网格线宽","Metalness":"金属度","Metallic":"金属质感","Reflectivity":"反射率","Specular":"镜面反射","Shine":"光泽","Cyan":"青色","Magenta":"品红","Alpha":"透明度","Source":"来源","Target":"目标","Input":"输入","Output":"输出","Preset":"预设","Difference":"差值",
  "Clamp":"钳制","Mirror":"镜像","Crop":"裁剪","Linear":"线性","Radial":"径向","Conic":"锥形","Angular":"角度","Horizontal":"横向","Vertical":"纵向","Both":"两者","Automatic":"自动","On":"开","Off":"关","Posterize":"色调分离","Quantize":"量化","Quantization":"量化","Quantization levels":"量化级数","Dither size":"抖动大小","Dither amount":"抖动量","Dither scale":"抖动缩放","Dither type":"抖动类型","Distortion scale":"扭曲缩放","Distortion amount":"扭曲量","Warp amount":"变形量","Warp scale":"变形缩放","Aspect ratio":"宽高比","Aspect":"宽高比","Iterations":"迭代次数","Steps":"步数","Bands":"色带数量","Number of colors":"颜色数量","Palette":"调色板","Gradient angle":"渐变角度","Gradient scale":"渐变缩放","Gradient start":"渐变起点","Gradient end":"渐变终点","Threshold low":"低阈值","Threshold high":"高阈值","Min":"最小值","Max":"最大值","Minimum":"最小值","Maximum":"最大值",
  "Shader effect":"着色器效果","Shader fill":"着色器填充","Custom effect":"自定义效果","Custom fill":"自定义填充","Created by me":"由我创建","In this file":"此文件中","New shader":"新建着色器","No results":"无结果","Search all tools":"搜索所有工具","Filter by source":"按来源筛选","Filter by category":"按分类筛选","Filter by price and type":"按价格和类型筛选","Clear search":"清除搜索","From Figma":"由 Figma 提供",
  "Agents":"智能体","Create new comment":"新建评论","Left sidebar":"左侧边栏","Right sidebar":"右侧边栏","Navigation Bar":"导航栏","Multiplayer tools":"协作工具","Prototype view":"原型视图","Apply variable mode":"应用变量模式","Toolbelt Mode":"工具栏模式","Creation tools":"创建工具","Type tools":"文本工具","Actions":"操作","Help/zoom toolbar":"帮助/缩放工具栏","Resize handle":"调整大小手柄","Loading":"正在加载","View mode":"查看模式","Show navigation":"显示导航","Show as grid":"网格显示","Show as list":"列表显示","Help and resources":"帮助与资源",
  "Use Smart Quotes/symbols":"使用智能引号和符号","Ctrl+click Opens Right Click Menus":"Ctrl+单击打开右键菜单","Use Old Shortcuts for Outlines":"使用旧版轮廓快捷键","Right-click and Drag to Pan":"右键拖动平移",
});
Object.assign(map,{
  "Expand image":"扩展图像","Erase object in image":"擦除图像中的对象","Isolate object in image":"分离图像中的对象","Remove object in image":"移除图像中的对象","Separate object in image":"分离图像中的对象","Upscale image":"提升图像分辨率","Enhance resolution":"提升分辨率","Edit image with a prompt":"使用提示词编辑图像","Use prompt to edit image":"使用提示词编辑图像","Make an image":"生成图像","Change lighting":"更改光照","Vectorize image":"图像矢量化","Crop image":"裁剪图像",
  "New chat":"新对话","Start new chat":"开始新对话","What do you want to do?":"你想做什么？","Design the next screen":"设计下一个界面","Design next screen":"设计下一个界面","Create variations":"创建变体","Create plugins and shaders":"创建插件和着色器","Chat history":"对话历史","Search chats":"搜索对话","No messages":"暂无消息","No matching chats":"没有匹配的对话","Not started":"尚未开始","Hide assistant":"隐藏智能体","Make all chats private":"将所有对话设为私密","New chat title":"新对话","Private":"私密","Last week":"上周","Earlier":"更早","Only mine":"仅我的","All chats":"所有对话","Retry":"重试","Undo changes":"撤销更改","Reject":"拒绝","Apply":"应用","Attach":"附加","Thinking":"思考中","Working":"处理中","Ask Figma":"询问 Figma","Figma Agent":"Figma 智能体","AI chat":"AI 对话",
  "Color filter":"色彩滤镜","Brilliance":"鲜明度","Definition":"清晰度","Lenticular":"柱状透镜","Zigzag":"锯齿","Wave":"波纹","Curved square":"曲边方形","Flat square":"平面方形","Edge wrap":"边缘环绕","Zero":"零值","Wrap":"环绕","No wrap":"不环绕","Transparent":"透明","Reflect":"反射","Border":"边界","Preserve aspect ratio":"保持宽高比","Nearest":"最近邻","Cubic":"三次插值","Bicubic":"双三次插值","Smoothstep":"平滑阶跃","Luminance":"亮度","Sepia":"棕褐色","Vintage":"复古","Cool":"冷色","Warm":"暖色","Vivid":"鲜艳","Dramatic":"戏剧化","Fade":"褪色","Noir":"黑白","Process":"冲印","Transfer":"转移","Original":"原始","Amount X":"横向量","Amount Y":"纵向量","Scale X":"横向缩放","Scale Y":"纵向缩放","Offset X":"横向偏移","Offset Y":"纵向偏移","Frequency X":"横向频率","Frequency Y":"纵向频率","Amplitude X":"横向振幅","Amplitude Y":"纵向振幅","Radius X":"横向半径","Radius Y":"纵向半径","Strength X":"横向强度","Strength Y":"纵向强度","Edge mode":"边缘模式","Wrap mode":"环绕模式","Interpolation":"插值方式","Color filter opacity":"色彩滤镜不透明度"
});
Object.assign(map,{
  "Color trim":"色彩裁剪","Average color":"平均颜色","Dissolve mode":"溶解模式","Dropout":"随机脱落","Knockout":"镂空",
  "Decay":"衰减","Shape mode":"形状模式","Sampling mode":"采样模式","Sample mode":"采样模式","Average":"平均值","Dominant color":"主色","Original color":"原始颜色",


  "Add min width…":"添加最小宽度…","Add max width…":"添加最大宽度…","Add min height…":"添加最小高度…","Add max height…":"添加最大高度…",
  "Remove min width":"移除最小宽度","Remove max width":"移除最大宽度","Remove min height":"移除最小高度","Remove max height":"移除最大高度",
  "Min width":"最小宽度","Max width":"最大宽度","Min height":"最小高度","Max height":"最大高度","Width mode":"宽度模式","Height mode":"高度模式",
  "Add images and files":"添加图像和文件","Attach Figma file":"附加 Figma 文件","Web search":"网页搜索","Skills":"技能",
  "Chats":"对话","Chat access":"对话访问权限","Delete chat":"删除对话","Shared chat":"共享对话","Available to all file editors":"所有文件编辑者均可访问","Private chat":"私密对话","Only you can access":"仅您可访问","Copy link to chat":"复制对话链接","Close agents panel":"关闭智能体面板","Open agents panel":"打开智能体面板","Make your existing chats private":"将现有对话设为私密",
  "Timing":"计时",
  "Motion":"动画","Timeline":"时间轴","Animation timeline":"动画时间轴","Open timeline":"打开时间轴","Close timeline":"关闭时间轴","Show timeline":"显示时间轴","Hide timeline":"隐藏时间轴",
  "Auto-keyframe":"自动关键帧","Turn on auto-keyframe":"开启自动关键帧","Turn off auto-keyframe":"关闭自动关键帧","Previous keyframe":"上一个关键帧","Next keyframe":"下一个关键帧","Add keyframe":"添加关键帧","Delete keyframe":"删除关键帧","Remove keyframe":"移除关键帧",
  "Turn on animation visibility":"开启动画可见性","Turn off animation visibility":"关闭动画可见性","Show animation":"显示动画","Hide animation":"隐藏动画","Play animation":"播放动画","Pause animation":"暂停动画","Loop playback":"循环播放","Rewind":"回到开头","Go to start":"转到开头","Go to end":"转到结尾",
  "Explore more on the timeline":"在时间轴中探索更多","Use the timeline view to interact with animations and see a breakdown of transitions.":"使用时间轴视图与动画交互，并查看过渡效果的详细分解。",
  "Static":"静态","Animated":"动画","Format":"格式","Frame rate":"帧速率","Export frame":"导出帧","Export animation":"导出动画","File size":"文件大小","Easing":"缓动","Ease in":"缓入","Ease out":"缓出","Ease in and out":"缓入缓出"
});
Object.assign(map,{
  "Shader":"着色器","Input mode":"输入模式","Luma":"亮度","Inverse luma":"反向亮度","Inverse alpha":"反向透明度","Edge softness":"边缘柔和度","Source mix":"源混合",
  "Grayscale":"灰度","High Contrast":"高对比度","Smoothing":"平滑度","Vignette":"暗角","Vignette Softness":"暗角柔和度","Spotlight":"聚光灯",
  "Sine wave":"正弦波","Twist":"扭转","Bulge":"膨胀","Pinch":"收缩","Ripple":"涟漪","Flag":"旗帜","Squeeze":"挤压","Swirl":"漩涡","Random":"随机度",
  "Full seat":"完整席位","Dev seat":"开发席位","Collab seat":"协作席位","Credits":"额度","AI credits":"AI 额度","Monthly credits":"每月额度","Credits reset":"额度重置",
  "Blended":"混合","BW light":"黑白（浅色）","BW dark":"黑白（深色）","Shader effects":"着色器效果","Shader Effects":"着色器效果","Generate code with context from this file via the remote MCP server.":"通过远程 MCP 服务器，结合此文件的上下文生成代码。","Generate code with context from this file via a remote MCP server.":"通过远程 MCP 服务器，结合此文件的上下文生成代码。","Generate code with context from this file via the desktop MCP server.":"通过桌面 MCP 服务器，结合此文件的上下文生成代码。"
});
delete map.CMYK;
delete map['BW light'];
delete map['BW dark'];
const ciMap=new Map(Object.entries(map).map(([key,value])=>[key.toLocaleLowerCase('en-US'),value]));
const shaderOnlyMap={"X":"横向","Y":"纵向","R":"半径","A":"角度","Light":"光照","Hex":"六边形","Hexagon":"六边形","Hexagonal":"六边形"};
const generatedLayerMap={frame:'画框',rectangle:'矩形',ellipse:'椭圆',polygon:'多边形',star:'星形',line:'直线',vector:'矢量',group:'组',section:'分区',component:'组件',instance:'实例',text:'文本'};
const fontWeightNames=new Set(['thin','hairline','extra light','ultra light','light','book','regular','roman','normal','medium','standard','semi bold','semibold','demi bold','demibold','bold','extra bold','extrabold','ultra bold','heavy','black','extra black','italic','oblique','bold italic']);
const colorSpaceNames=new Set(['rgb','cmyk','bw light','bw dark','srgb','linear','oklab','lab','lch','oklch','display p3','p3','hsl','hsv','hsb']);
const shaderQualityNames=new Set(['low','medium','high']);
const iphoneDeviceColorTranslations={black:'黑色',white:'白色'};
const themeOptionTranslations={dark:'黑色',light:'浅色'};
const iphoneDeviceColorPaletteSignatures=[
  [['lavender','薰衣草紫色'],['sage','鼠尾草绿色'],['mist blue','青雾蓝色']],
  [['pink','粉色'],['teal','深青色'],['ultramarine','群青色']],
  [['blue','蓝色'],['green','绿色'],['pink','粉色'],['yellow','黄色']],
  [['black','黑色'],['white','白色']]
];
const compareListParameterNames=new Set([
  'fill type','opacity','color stop 1','color stop 2','color','x','y','blur','spread','fill','blend mode','visibility','effect type',
  'position','width','height','rotation','corner radius','radius','layer type','constraints','horizontal constraint','vertical constraint',
  'stroke','stroke type','stroke paint','stroke weight','stroke align','dash pattern','cap','join','miter angle',
  'font name','font family','font style','font weight','font size','line height','letter spacing','text content','text align','paragraph spacing',
  'layout','layout mode','item spacing','padding','padding top','padding right','padding bottom','padding left','alignment','clipping',
  'shadow','inner shadow','drop shadow','effect','image','scale mode','crop','exposure','contrast','saturation','temperature','tint'
]);
const shaderPanelSelector='[data-testid="effects-settings-picker"],[data-testid*="custom-paint-prop-editor"],[data-testid*="shader"]';
const componentPropertyControlSelector='[data-testid*="component-property" i],[data-testid*="component_property" i],[data-testid*="variant-property" i],[data-testid*="variant_property" i],[data-testid*="variant-select" i],[data-testid*="instance-property" i],[aria-label*="component property" i],[aria-label*="variant property" i],[class*="component_property" i],[class*="component-property" i],[class*="variant_property" i],[class*="variant-property" i]';
const componentUserNameSelector='[data-testid*="component-name" i],[data-testid*="variant-name" i],[aria-label*="component name" i],[aria-label*="variant name" i]';
const componentNodeSelector='[data-node-type="COMPONENT" i],[data-node-type="INSTANCE" i]';
const componentNameInputSelector='[data-testid*="component-name" i],[data-testid*="variant-name" i],[data-testid*="component-property-name" i],[data-testid*="variant-property-name" i],[data-testid*="component-property-option" i],[data-testid*="variant-option" i],[data-testid*="instance-property-option" i],[aria-label*="component name" i],[aria-label*="variant name" i]';
const componentPropertyValueSignatures=[['listening'],['speaking'],['offline'],['disabled','已禁用'],['thinking','思考中'],['idle'],['loading','正在加载','加载中']];
const componentPropertyValuePairSignatures=[[['default','默认'],['scrolled']]];
const directTooltipTranslations={'Bold':'加粗','Italic':'斜体','Strikethrough':'删除线','Header 1':'标题 1','Link':'链接','Code':'代码','Code block':'代码块','Standard':'标准'};
const textFormattingMenuTranslations={bold:'加粗',italic:'斜体'};
const componentDetailActionTranslations=new Set(['insert instance','reset properties']);
const compactTooltipSourceLabels=new Set(['Create component','Visual search']);
const compactMotionTooltipAttribute='data-figma-cn-compact-motion-tooltip';
const controlledAutoInputs=new WeakSet();
const controlledAutoOverlayAttribute='data-figma-cn-controlled-auto';
const controlledAutoOverlayStyleId='figma-cn-controlled-auto-style';
const attrs=['aria-label','aria-description','aria-valuetext','data-label','data-tooltip','placeholder','title','alt'];
const connectedTeamFragments=[
  ['connected folder','已连接文件夹'],
  ['Everyone in it will be able to edit files and','项目中的所有人都可以编辑文件并'],
  ['use libraries.','使用组件库。']
];
const folderTransferFragments=[
  ['You can transfer this folder and its content to an external team in a','你可以将此文件夹及其内容转移到使用'],
  ['You can transfer a copy of this folder to another team in a','你可以将此文件夹的副本转移到使用']
];
function isComponentUserNameContext(element){
  if(!element)return false;
  if(element.matches?.(componentUserNameSelector)||element.parentElement?.matches?.(componentUserNameSelector))return true;
  const node=element.closest?.(componentNodeSelector);
  if(!node)return false;
  const value=(element.value??element.textContent??'').trim().replace(/\s+/g,' ');
  const nodeText=(node.textContent||'').trim().replace(/\s+/g,' ');
  return !!value&&value===nodeText;
}
function isComponentNameInputContext(element){
  if(!element)return false;
  if(element.matches?.(componentNameInputSelector))return true;
  const parent=element.parentElement;
  if(!parent?.matches?.(componentNameInputSelector))return false;
  return parent.matches?.('input,textarea,[contenteditable="true"],[role="textbox"]')||parent.isContentEditable===true;
}
const shortMonths={jan:1,january:1,feb:2,february:2,mar:3,march:3,apr:4,april:4,may:5,jun:6,june:6,jul:7,july:7,aug:8,august:8,sep:9,september:9,oct:10,october:10,nov:11,november:11,dec:12,december:12};
function localizeShortDate(value){
  const match=value.trim().match(/^(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{1,2})(?:,\s*(\d{4}))?$/i);
  return match?`${match[3]?`${match[3]}年`:''}${shortMonths[match[1].toLowerCase()]}月${match[2]}日`:value.trim();
}
function localizeClockTime(value){
  const normalized=value.trim().replace(/\s+/g,' ');
  let match=normalized.match(/^(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{1,2})(?:,\s*(\d{4}))?,\s*(\d{1,2}:\d{2})\s*(AM|PM)$/i);
  if(match)return `${match[3]?`${match[3]}年`:''}${shortMonths[match[1].toLowerCase()]}月${match[2]}日 ${match[5].toUpperCase()==='AM'?'上午':'下午'} ${match[4]}`;
  match=normalized.match(/^(\d{1,2}:\d{2})\s*(AM|PM)$/i);
  return match?`${match[2].toUpperCase()==='AM'?'上午':'下午'} ${match[1]}`:undefined;
}
function localizeRelativeTime(value){
  const normalized=value.trim().replace(/\s+/g,' ');
  let match=normalized.match(/^(a|an|\d+)\s+(second|minute|hour|day|week|month|year)s?\s+ago$/i);
  if(match){
    const count=/^(?:a|an)$/i.test(match[1])?'1':match[1];
    const unit={second:'秒',minute:'分钟',hour:'小时',day:'天',week:'周',month:'个月',year:'年'}[match[2].toLowerCase()];
    return `${count} ${unit}前`;
  }
  match=normalized.match(/^(\d+)\s*([smhdw])\s+ago$/i);
  return match?`${match[1]} ${{s:'秒',m:'分钟',h:'小时',d:'天',w:'周'}[match[2].toLowerCase()]}前`:undefined;
}
function isFontWeightContext(element,normalized){
  if(!element||!fontWeightNames.has(normalized.toLocaleLowerCase('en-US')))return false;
  const weightControl=element.closest?.('[aria-label*="font weight" i],[aria-label*="font style" i],[data-testid*="font-weight" i],[data-testid*="font_weight" i],[class*="font_weight" i],[class*="font-weight" i]');
  if(weightControl)return true;
  const popup=element.closest?.('[role="listbox"],[role="menu"]');
  if(!popup)return false;
  const weights=new Set();
  for(const item of popup.querySelectorAll('[role="option"],[role="menuitem"],[role="menuitemradio"],[role="menuitemcheckbox"],button')){
    const label=(item.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(fontWeightNames.has(label))weights.add(label);
  }
  if(weights.size>=2)return true;
  return false;
}
function localizeDirectTooltip(element,normalized){
  if(!element?.hasAttribute?.('data-tooltip'))return undefined;
  for(const [label,translated] of Object.entries(directTooltipTranslations)){
    if(normalized===label||normalized.startsWith(`${label} `))return translated+normalized.slice(label.length);
  }
  return undefined;
}
function localizeTextFormattingMenu(element,normalized){
  const translated=textFormattingMenuTranslations[normalized.toLocaleLowerCase('en-US')];
  if(!element||!translated)return undefined;
  const menu=element.closest?.('[role="menu"]');
  if(!menu)return undefined;
  const text=(menu.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
  const hasUnderline=text.includes('underline')||text.includes('下划线');
  const hasStrikethrough=text.includes('strikethrough')||text.includes('删除线');
  const hasCreateLink=text.includes('create link')||text.includes('创建链接');
  return hasUnderline&&hasStrikethrough&&hasCreateLink?translated:undefined;
}
function isIPhoneDeviceColorContext(element,normalized){
  const color=normalized.toLocaleLowerCase('en-US');
  if(!element||!iphoneDeviceColorTranslations[color])return false;
  let container=element;
  for(let depth=0;container&&depth<9;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text||text.length>500)continue;
    const matches=iphoneDeviceColorPaletteSignatures.some((signature)=>signature.every((variants)=>variants.some((label)=>text.includes(label))));
    if(matches)return true;
  }
  return false;
}
function isThemeOptionContext(element,normalized){
  if(!element||!themeOptionTranslations[normalized.toLocaleLowerCase('en-US')])return false;
  let container=element;
  for(let depth=0;container&&depth<9;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text||text.length>300)continue;
    const hasLight=text.includes('light')||text.includes('浅色');
    const hasDark=text.includes('dark')||text.includes('深色');
    const hasSystem=text.includes('system theme')||text.includes('系统主题');
    if(hasLight&&hasDark&&hasSystem)return true;
  }
  return false;
}
function isAccountThemeContext(element,normalized){
  if(!element||!themeOptionTranslations[normalized.toLocaleLowerCase('en-US')])return false;
  let container=element;
  for(let depth=0;container&&depth<9;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text||text.length>800)continue;
    const hasTheme=text.includes('theme')||text.includes('主题');
    const hasContrast=text.includes('enhance contrast')||text.includes('增强对比度');
    if(hasTheme&&hasContrast)return true;
  }
  return false;
}
function isLibraryModeNameContext(element,normalized){
  const name=normalized.toLocaleLowerCase('en-US');
  if(!element||(name!=='dark'&&name!=='light'))return false;
  let container=element;
  for(let depth=0;container&&depth<9;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text||text.length>300)continue;
    const hasAuto=/(?:auto|自动)\s*[\(（][^)）]+[\)）]/i.test(text);
    const hasMode=/(?:^|\s)(?:mode|模式)(?:\s|$)/i.test(text);
    if(hasAuto&&hasMode)return true;
  }
  return false;
}
function isBlendModeNormalContext(element,normalized){
  if(!element||normalized.toLocaleLowerCase('en-US')!=='normal')return false;
  if(element.closest?.('[aria-label*="blend mode" i],[data-testid*="blend-mode" i],[data-testid*="blend_mode" i]'))return true;
  let row=element;
  for(let depth=0;row&&depth<5;depth++,row=row.parentElement){
    const rowText=(row.textContent||'').trim().replace(/\s+/g,' ');
    if(rowText.length<=120&&/(?:blend mode|混合模式)/i.test(rowText))return true;
  }
  const popup=element.closest?.('[role="listbox"],[role="menu"]');
  if(!popup)return false;
  const text=(popup.textContent||'').toLocaleLowerCase('en-US');
  const markers=['multiply','screen','overlay','soft light','hard light','difference','exclusion','hue','saturation','luminosity','正片叠底','滤色','叠加','柔光','强光','差值','排除','色相','饱和度','明度'];
  let hits=0;
  for(const marker of markers){if(text.includes(marker)&&++hits>=3)return true;}
  return false;
}
function isCodeSyntaxContext(element){
  if(!element)return false;
  if(element.closest?.('pre,code,[role="code"]'))return true;
  let current=element;
  for(let depth=0;current&&depth<4;depth++,current=current.parentElement){
    const text=(current.textContent||'').trim().replace(/\s+/g,' ');
    if(text.length<=300&&/(?:^|\s)(?:--[\w-]+|[a-z-]+)\s*:\s*[^;{}]+;/i.test(text))return true;
  }
  return false;
}
function isCompareListParameterContext(element,normalized){
  if(!element||!compareListParameterNames.has(normalized.toLocaleLowerCase('en-US')))return false;
  const dialog=element.closest?.('[role="dialog"]');
  if(dialog){
    const text=(dialog.textContent||'').replace(/\s+/g,' ');
    if(/(?:Compare changes|对比变更)/i.test(text)&&/(?:List|列表)/i.test(text)&&/(?:Code|代码)/i.test(text))return true;
  }
  let current=element;
  for(let depth=0;current&&depth<24;depth++,current=current.parentElement){
    const text=(current.textContent||'').replace(/\s+/g,' ');
    if(/(?:Compare changes|对比变更)/i.test(text)&&/(?:List|列表)/i.test(text)&&/(?:Code|代码)/i.test(text))return true;
  }
  return false;
}
function isComponentPropertyValueContext(element){
  if(!element)return false;
  if(element.closest?.(componentPropertyControlSelector))return true;
  const popup=element.closest?.('[role="listbox"],[role="menu"]');
  if(popup&&(popup.matches?.(componentPropertyControlSelector)||popup.querySelector?.(componentPropertyControlSelector)))return true;
  const popupId=popup?.getAttribute?.('id');
  if(popupId){
    const escaped=globalThis.CSS?.escape?CSS.escape(popupId):popupId.replace(/["\\]/g,'\\$&');
    const trigger=document.querySelector?.(`[aria-controls="${escaped}"],[aria-owns="${escaped}"]`);
    if(trigger?.closest?.(componentPropertyControlSelector))return true;
  }
  let container=popup||element;
  for(let depth=0;container&&depth<6;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text||text.length>400)continue;
    for(const signature of componentPropertyValuePairSignatures){if(signature.every((variants)=>variants.some((label)=>text.includes(label))))return true;}
    let hits=0;
    for(const variants of componentPropertyValueSignatures){if(variants.some((label)=>text.includes(label))&&++hits>=5)return true;}
  }
  return false;
}
function isLayoutGuideCountAutoContext(element,normalized){
  if(!element||!/^(?:auto|自动)$/i.test(normalized))return false;
  if(element.closest?.('[class*="grid_settings--stripesNumSections--"]'))return true;
  const dialog=element.closest?.('[role="dialog"]');
  if(!dialog)return false;
  const text=(dialog.textContent||'').trim().replace(/\s+/g,' ');
  const hasGuideType=/(?:Layout guide type|布局参考线类型)/i.test(text);
  const hasAmount=/(?:Amount|数量)/i.test(text);
  const hasMargins=/(?:Margins|边距)/i.test(text);
  const hasGutter=/(?:Gutter|间距)/i.test(text);
  return hasGuideType&&hasAmount&&hasMargins&&hasGutter;
}
function isTypographyLineHeightAutoInput(element){
  if(element?.tagName!=='INPUT'||!element.closest?.('[class*="type_panel--"]'))return false;
  return /^(?:Line height|行高)$/i.test((element.getAttribute('aria-label')||'').trim());
}
function isExportQualityContext(element,normalized){
  if(!element||!shaderQualityNames.has(normalized.toLocaleLowerCase('en-US')))return false;
  if(element.closest?.('[aria-label*="quality" i],[data-testid*="quality" i],[class*="quality" i]'))return true;
  let container=element;
  for(let depth=0;container&&depth<9;depth++,container=container.parentElement){
    const text=(container.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
    if(!text)continue;
    if(text.length<=220&&(text.includes('quality')||text.includes('质量')))return true;
    if(text.length>1800)continue;
    const hasExport=text.includes('export')||text.includes('导出');
    const hasQuality=text.includes('quality')||text.includes('质量');
    const hasExportOptions=/(?:image resampling|图像重采样|color profile|颜色描述文件|suffix|后缀)/i.test(text);
    if(hasExport&&hasQuality&&hasExportOptions)return true;
  }
  return false;
}
function isShaderQualityPopup(element,normalized){
  if(!element||!shaderQualityNames.has(normalized.toLocaleLowerCase('en-US')))return false;
  let container=element;
  for(let depth=0;container&&depth<6;depth++,container=container.parentElement){
    const text=container.textContent||'';
    if(text.length>1000)break;
    const levels=new Set();
    for(const item of container.querySelectorAll('[role="option"],[role="menuitem"],[role="menuitemradio"],[data-value],button,span,div')){
      const label=(item.textContent||'').trim().replace(/\s+/g,' ').toLocaleLowerCase('en-US');
      if(label==='low'||label==='低')levels.add('low');
      else if(label==='medium'||label==='中')levels.add('medium');
      else if(label==='high'||label==='高')levels.add('high');
    }
    if(levels.size===3)return true;
  }
  return false;
}
function isColorSpaceContext(element,normalized){
  if(!element||!colorSpaceNames.has(normalized.toLocaleLowerCase('en-US')))return false;
  const panel=element.closest?.(shaderPanelSelector);
  if(panel&&/(?:Mix space|混合空间|Color space|色彩空间)/i.test(panel.textContent||''))return true;
  const popup=element.closest?.('[role="listbox"],[role="menu"]');
  if(!popup)return false;
  const popupText=(popup.textContent||'').toLocaleLowerCase('en-US');
  let hits=0;
  for(const name of colorSpaceNames){if(popupText.includes(name)&&++hits>=2)return true;}
  return false;
}
function isTrashDialogContext(element){
  let container=element;
  for(let depth=0;container&&depth<12;depth++,container=container.parentElement){
    const text=(container.textContent||'').replace(/\s+/g,' ').trim();
    if(text.length>1200)continue;
    if(/(?:Move file to trash|将文件移到回收站)/i.test(text))return true;
    if(/(?:You're about to move the file|You can always restore it later from|Anyone with edit access on the project|你即将把文件|项目中任何拥有编辑权限)/i.test(text)&&/(?:trash|to trash|回收站|恢复该文件)/i.test(text))return true;
  }
  return false;
}
function lookup(value,node){
  if(typeof value!=='string')return undefined;
  const leading=value.match(/^\s*/)?.[0]??'',trailing=value.match(/\s*$/)?.[0]??'',trimmed=value.trim(),normalized=trimmed.replace(/[\u200B-\u200F\u2060\uFEFF]/g,'').replace(/\s+/g,' ');
  if(!/[A-Za-z0-9]/.test(normalized))return undefined;
  const element=node?.nodeType===Node.ELEMENT_NODE?node:node?.parentElement;
  const blendModeNormal=isBlendModeNormalContext(element,normalized);
  const iphoneDeviceColor=isIPhoneDeviceColorContext(element,normalized);
  const themeOption=isThemeOptionContext(element,normalized)||isAccountThemeContext(element,normalized);
  const libraryModeName=isLibraryModeNameContext(element,normalized);
  const layoutGuideCountAuto=isLayoutGuideCountAutoContext(element,normalized);
  const directTooltip=localizeDirectTooltip(element,normalized);
  const textFormattingMenu=localizeTextFormattingMenu(element,normalized);
  const trashDialogCandidate=/^(?:You're about to move the file|to trash\.|You can always restore it later from the|Anyone with edit access on the project can restore it later from the|(?:the )?Trash section\.|section\.)$/i.test(normalized);
  const trashDialog=trashDialogCandidate&&isTrashDialogContext(element);
  const componentNameValue=isComponentNameInputContext(element);
  const ambiguousThemeName=normalized.toLocaleLowerCase('en-US')==='dark'||normalized.toLocaleLowerCase('en-US')==='light';
  const relativeTime=/^\d+\s+(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?)\s+ago$/i.test(normalized)||/^\d+\s*[smhdw]\s+ago$/i.test(normalized);
  if(componentNameValue)return undefined;
  const isComponentDetailText=componentDetailActionTranslations.has(normalized.toLocaleLowerCase('en-US'))||/^includes\s+\d+\s+variants?$/i.test(normalized);
  if(isComponentPropertyValueContext(element)&&!layoutGuideCountAuto&&!directTooltip&&!isComponentDetailText&&normalized.toLocaleLowerCase('en-US')!=='mixed')return undefined;
  if(isCompareListParameterContext(element,normalized))return undefined;
  if(isCodeSyntaxContext(element)&&!blendModeNormal&&!relativeTime&&!directTooltip)return undefined;
  if(libraryModeName||ambiguousThemeName&&!themeOption)return undefined;
  const shaderQuality=shaderQualityNames.has(normalized.toLocaleLowerCase('en-US'))&&(element?.closest?.(shaderPanelSelector)||isShaderQualityPopup(element,normalized));
  const exportQuality=isExportQualityContext(element,normalized);
  if(!blendModeNormal&&!shaderQuality&&!exportQuality&&!iphoneDeviceColor&&!themeOption&&!directTooltip&&!textFormattingMenu&&isFontWeightContext(element,normalized))return undefined;
  if(isColorSpaceContext(element,normalized))return undefined;
  let translated=directTooltip??textFormattingMenu??map[trimmed]??map[normalized]??ciMap.get(trimmed.toLocaleLowerCase('en-US'))??ciMap.get(normalized.toLocaleLowerCase('en-US'));
  if(!translated&&trashDialog){
    if(/^You're about to move the file$/i.test(normalized))translated='你即将把文件';
    else if(/^to trash\.$/i.test(normalized))translated='移到回收站。';
    else if(/^You can always restore it later from the$/i.test(normalized))translated='你可以随时从';
    else if(/^Anyone with edit access on the project can restore it later from the$/i.test(normalized))translated='项目中任何拥有编辑权限的人都可以稍后从';
    else if(/^(?:the )?Trash section\.$/i.test(normalized))translated='回收站中恢复该文件。';
    else if(/^section\.$/i.test(normalized))translated='中恢复该文件。';
  }
  if(blendModeNormal)translated='正常';
  if(iphoneDeviceColor)translated=iphoneDeviceColorTranslations[normalized.toLocaleLowerCase('en-US')];
  if(themeOption)translated=themeOptionTranslations[normalized.toLocaleLowerCase('en-US')];
  if(layoutGuideCountAuto)translated='自动';
  if(translated===trimmed||translated===normalized)translated=undefined;
  if(element?.closest?.(shaderPanelSelector)&&shaderOnlyMap[normalized])translated=shaderOnlyMap[normalized];
  if(!translated){
    let match;
    const localizedClockTime=localizeClockTime(normalized);
    const localizedDate=localizeShortDate(normalized);
    if(localizedClockTime)translated=localizedClockTime;
    else if(localizedDate!==normalized)translated=localizedDate;
    else if(match=normalized.match(/^More actions for (.+)$/i))translated=`更多操作：${lookup(match[1],node)??match[1]}`;
    else if(match=normalized.match(/^Updates from\s+(.+)$/i))translated=`来自 ${match[1]} 的更新`;
    else if(match=normalized.match(/^Results for\s+(.+)$/i))translated=`${match[1]} 的结果`;
    else if(match=normalized.match(/^Published by\s+(.+)$/i))translated=`发布者：${match[1]}`;
    else if(match=normalized.match(/^Added by\s+(.+)$/i))translated=`添加者：${match[1]}`;
    else if(match=normalized.match(/^([\d.,]+[kKmM]?) users?$/))translated=`${match[1]} 位用户`;
    else if(match=normalized.match(/^(\d+)\s+selected$/i))translated=`已选择 ${match[1]} 个`;
    else if(match=normalized.match(/^Account dropdown for (.+)$/))translated=`${match[1]} 的账户菜单`;
    else if(match=normalized.match(/^Move file\s+(.+)$/i))translated=`移动文件 ${match[1]}`;
    else if(match=normalized.match(/^Plan: (.+)$/))translated=`方案：${match[1]}`;
    else if(match=normalized.match(/^Change plan\. (.+) is currently on the (.+) plan$/))translated=`更改方案。${match[1]} 当前使用${match[2]}方案`;
    else if(match=normalized.match(/^Solid color hex: (.+)$/))translated=`纯色十六进制：${match[1]}`;
    else if(match=normalized.match(/^Fixed (width|height)(?:\s*\(([^)]+)\))?$/i))translated=`固定${match[1].toLowerCase()==='width'?'宽度':'高度'}${match[2]?`（${match[2]}）`:''}`;
    else if(match=normalized.match(/^(Hug contents|Fill container)(?:\s*\(([^)]+)\))?$/i))translated=`${match[1].toLowerCase()==='hug contents'?'适应内容':'填充容器'}${match[2]?`（${match[2]}）`:''}`;
    else if(match=normalized.match(/^(Add|Remove) (min|max) (width|height)(?:\.{3}|…)?$/i))translated=`${match[1].toLowerCase()==='add'?'添加':'移除'}${match[2].toLowerCase()==='min'?'最小':'最大'}${match[3].toLowerCase()==='width'?'宽度':'高度'}${match[1].toLowerCase()==='add'?'…':''}`;
    else if(match=normalized.match(/^(\d+)\s+(columns?|rows?)$/i))translated=`${match[1]} ${match[2].toLowerCase().startsWith('column')?'列':'行'}`;
    else if(match=normalized.match(/^Collection\s+(\d+)$/i))translated=`集合 ${match[1]}`;
    else if(match=normalized.match(/^(\d+)\s+autosave\s+versions?$/i))translated=`${match[1]} 个自动保存版本`;
    else if(match=normalized.match(/^Component instance(\s*\(.+\))$/i))translated=`组件实例${match[1]}`;
    else if(match=normalized.match(/^On this page,\s*([\d,]+)\s+layers have been updated to the latest auto layout without any visible changes\.$/i))translated=`在此页面中，${match[1]} 个图层已更新为最新自动布局，且没有任何可见变化。`;
    else if(match=normalized.match(/^There are\s+([\d,]+)\s+layers that might shift with the new layout\. You can review them below before updating\.$/i))translated=`有 ${match[1]} 个图层可能会随新布局发生偏移。你可以在更新前查看下方内容。`;
    else if(match=normalized.match(/^You['’]re about to move the file\s+(.+?)\s+to trash\.$/i))translated=`你即将把文件 ${match[1]} 移到回收站。`;
    else if(match=normalized.match(/^You can always restore it later from the (?:Trash|回收站) section\.$/i))translated='你可以随时从回收站中恢复该文件。';
    else if(match=normalized.match(/^(\d+)\s+layers?$/i))translated=`${match[1]} 个图层`;
    else if(match=normalized.match(/^Rename\s+(\d+)\s+layers?$/i))translated=`重命名 ${match[1]} 个图层`;
    else if(match=normalized.match(/^Reset\s+["“](.+)["”]$/i))translated=`重置“${match[1]}”`;
    else if(match=normalized.match(/^(\d+)\s+of\s+(\d+)$/i))translated=`${match[1]} / ${match[2]}`;
    else if(match=normalized.match(/^Step\s+(\d+)\s+of\s+(\d+)$/i))translated=`第 ${match[1]} 步，共 ${match[2]} 步`;
    else if(match=normalized.match(/^See all\s+(\d+)\s+colors?$/i))translated=`查看全部 ${match[1]} 种颜色`;
    else if(match=normalized.match(/^Show\s+(\d+)\s+more lines? of code$/i))translated=`再显示 ${match[1]} 行代码`;
    else if(match=normalized.match(/^Includes\s+(\d+)\s+variants?$/i))translated=`包含 ${match[1]} 个变体`;
    else if(match=normalized.match(/^Loading\s+(\d+)\s+pages\s+for\s+plugin(?:\.{3}|…)?$/i))translated=`正在为插件加载 ${match[1]} 个页面…`;
    else if(match=normalized.match(/^Updating assets\s+(\d+)\s*\/\s*(\d+)$/i))translated=`正在更新资源 ${match[1]} / ${match[2]}`;
    else if(match=normalized.match(/^Resized height to\s+(.+)$/i))translated=`高度已调整为 ${match[1]}`;
    else if(match=normalized.match(/^Worked\s+for\s+(\d+)s$/i))translated=`运行了 ${match[1]} 秒`;
    else if(match=normalized.match(/^This prompt uses\s+(\d+)\s+AI credits while in beta\. After beta, it will use\s+(\d+)\s+credits\.$/i))translated=`此提示在测试期间使用 ${match[1]} 点 AI 额度。测试结束后将使用 ${match[2]} 点额度。`;
    else if(match=normalized.match(/^Sweep\s+(-?\d+(?:\.\d+)?)%$/i))translated=`圆弧范围 ${match[1]}%`;
    else if(match=normalized.match(/^Connection error:\s*(-?\d+)$/i))translated=`连接错误：${match[1]}`;
    else if(match=normalized.match(/^Error code:\s*(-?\d+)$/i))translated=`错误代码：${match[1]}`;
    else if(match=normalized.match(/^Error navigating to ['“](.+?)['”]:\s*(ERR_[A-Z0-9_]+)$/i))translated=`导航至“${match[1]}”时出错：${match[2]}`;
    else if(match=normalized.match(/^Auto\s*\((.+)\)$/i))translated=`自动（${match[1]}）`;
    else if(match=normalized.match(/^Turn (on|off) auto-keyframe(?:\s+(.+))?$/i))translated=`${match[1].toLowerCase()==='on'?'开启':'关闭'}自动关键帧${match[2]?` ${match[2]}`:''}`;
    else if(match=normalized.match(/^([\d,.]+)\s+credits?\s+left$/i))translated=`剩余 ${match[1]} 点额度`;
    else if(match=normalized.match(/^Edited\s+(.+)$/i))translated=`编辑于 ${localizeRelativeTime(match[1])??match[1]}`;
    else if(match=normalized.match(/^Viewed\s+(\d+)\s+(seconds?|minutes?|hours?|days?|weeks?|months?|years?)\s+ago$/i)){
      const unit={second:'秒',minute:'分钟',hour:'小时',day:'天',week:'周',month:'个月',year:'年'}[match[2].toLowerCase().replace(/s$/,'')];
      translated=`${match[1]}${unit}前查看`;
    }
    else if(match=normalized.match(/^by\s+([^,.!?]{1,80})$/i))translated=`作者：${match[1]}`;
    else if(match=normalized.match(/^Try in(?:\s+(.+?))?(?:\.{3}|…)?$/i))translated=match[1]?`在 ${match[1]} 中试用`:'试用…';
    else if(match=normalized.match(/^(\d+)\s+(?:person|people)\s+using\s+AI\s+credits$/i))translated=`${match[1]} 人正在使用 AI 额度`;
    else if(match=normalized.match(/^(\d+)\s+(?:person|people)\s+out\s+of\s+seat\s+credits$/i))translated=`${match[1]} 人的席位额度已用完`;
    else if(match=normalized.match(/^([\d,.]+)\s+AI\s+credits\/month\s+for\s+Full\s+seats$/i))translated=`完整席位每月包含 ${match[1]} 点 AI 额度`;
    else if(match=normalized.match(/^(\d+)\s*([smhdw])\s+ago$/i))translated=`${match[1]}${{s:'秒',m:'分钟',h:'小时',d:'天',w:'周'}[match[2].toLowerCase()]}前`;
    else if(match=normalized.match(/^(\d+)\s+(seconds?|minutes?|hours?|days?|weeks?|months?|years?)\s+ago$/i)){
      const unit={second:'秒',minute:'分钟',hour:'小时',day:'天',week:'周',month:'个月',year:'年'}[match[2].toLowerCase().replace(/s$/,'')];
      translated=`${match[1]}${unit}前`;
    }
    else if(localizeRelativeTime(normalized))translated=localizeRelativeTime(normalized);
    else if(match=normalized.match(/^(\d+)\s+available$/i))translated=`${match[1]} 个可用`;
    else if(match=normalized.match(/^Renews\s+(.+?)(?:\s*·)?$/i))translated=`续订日期：${localizeShortDate(match[1])}`;
    else if(match=normalized.match(/^Next monthly invoice due\s+(.+)$/i))translated=`下期月度账单将于${localizeShortDate(match[1])}到期`;
    else if(match=normalized.match(/^These drafts belonged to users who were removed from ([^,]+),\s*(.*)$/i))translated=`这些草稿属于已从 ${match[1]} 中移除的用户，${match[2]}`;
    else if(match=normalized.match(/^Reset:\s+(.+)$/i))translated=`重置：${localizeShortDate(match[1])}`;
    else if(match=normalized.match(/^Your (Full|Dev|Collab|View) seat in (.+?),? comes with ([\d,.]+) credits? a month\. Your seat credits reset (.+)\.$/i)){
      const seat={full:'完整',dev:'开发',collab:'协作',view:'查看'}[match[1].toLowerCase()]??match[1];
      translated=`${match[2]} 中的${seat}席位每月包含 ${match[3]} 点额度。席位额度将于${localizeShortDate(match[4])}重置。`;
    }
    else if(match=normalized.match(/^Export Frame\s+(.+)$/i))translated=`导出画框 ${match[1]}`;
    else if(match=normalized.match(/^Go to Frame\s+(.+)$/i))translated=`转到画框 ${match[1]}`;
    else if(match=normalized.match(/^Export\s+(.+)$/i))translated=`导出 ${match[1]}`;
    else if(match=normalized.match(/^(Frame|Rectangle|Ellipse|Polygon|Star|Line|Vector|Group|Section|Component|Instance|Text)\s+(\d+)$/i))translated=`${generatedLayerMap[match[1].toLowerCase()]} ${match[2]}`;
    else if(match=normalized.match(/^Page\s+(\d+)$/i))translated=`页面 ${match[1]}`;
    else if(match=normalized.match(/^Grid\s+(\d+(?:\.\d+)?(?:px|pt)?)$/i))translated=`网格 ${match[1]}`;
  }
  return translated&&translated!==trimmed&&translated!==normalized?leading+translated+trailing:undefined;
}
function compactTranslatedTooltip(node,value){
  if(!compactTooltipSourceLabels.has(value.trim()))return;
  let tooltip=node.parentElement;
  for(let depth=0;tooltip&&depth<8;depth+=1,tooltip=tooltip.parentElement){
    const tooltipIsContent=tooltip.getAttribute?.('role')==='tooltip'||[...(tooltip.classList??[])].some(name=>name.startsWith('tooltip--content--'));
    if(!tooltipIsContent)continue;
    const oldWidth=tooltip.getBoundingClientRect().width;
    const oldLeft=Number.parseFloat(tooltip.style.left);
    tooltip.style.width='max-content';
    tooltip.style.minWidth='0';
    const newWidth=tooltip.getBoundingClientRect().width;
    if(Number.isFinite(oldLeft)&&newWidth<oldWidth)tooltip.style.left=`${oldLeft+(oldWidth-newWidth)/2}px`;
    return;
  }
}
function compactMotionBetaTooltip(node){
  const element=node?.nodeType===Node.ELEMENT_NODE?node:node?.parentElement;
  const tooltip=element?.closest?.('[data-is-the-tooltip]');
  if(!tooltip||tooltip.hasAttribute(compactMotionTooltipAttribute))return;
  const normalized=(tooltip.textContent||'').replace(/\s+/g,'');
  if(normalized!=='动画测试')return;
  const oldWidth=tooltip.getBoundingClientRect().width;
  const oldLeft=Number.parseFloat(tooltip.style.left);
  tooltip.style.width='max-content';
  tooltip.style.minWidth='0';
  const newWidth=tooltip.getBoundingClientRect().width;
  tooltip.style.setProperty('width',getComputedStyle(tooltip).width,'important');
  tooltip.setAttribute(compactMotionTooltipAttribute,'');
  if(Number.isFinite(oldLeft)&&newWidth<oldWidth)tooltip.style.left=`${oldLeft+(oldWidth-newWidth)/2}px`;
}
function applyTranslatedText(node,value,translated){
  if(value.trim()==='Pen'&&translated.trim()==='钢笔'){
    const element=node.parentElement;
    element?.style?.setProperty('white-space','nowrap');
    element?.style?.setProperty('word-break','keep-all');
  }
  node.nodeValue=translated;
  compactTranslatedTooltip(node,value);
}
function isConnectedTeamContext(element){
  for(let current=element,depth=0;current&&depth<10;current=current.parentElement,depth++){
    const text=(current.textContent||'').replace(/\s+/g,' ').trim();
    if(text.length>1400)continue;
    if(/(?:Connect an external team|连接外部团队|connected (?:project|folder)|已连接(?:项目|文件夹))/i.test(text))return true;
  }
  return false;
}
function isFolderTransferContext(element){
  for(let current=element,depth=0;current&&depth<10;current=current.parentElement,depth++){
    const text=(current.textContent||'').replace(/\s+/g,' ').trim();
    if(text.length>2400)continue;
    if(/(?:Start transfer|Transfer a copy|开始转让|转让副本)/i.test(text)&&/(?:nested folders|嵌套文件夹)/i.test(text))return true;
  }
  return false;
}
function translateFolderTransferPlanElement(element){
  if(!element||element.nodeType!==Node.ELEMENT_NODE)return;
  const text=(element.textContent||'').replace(/\s+/g,' ').trim();
  const match=text.match(/^Professional, Organization, or Enterprise plan(\.)?$/);
  if(!match||!isFolderTransferContext(element))return;
  element.textContent=`专业版、组织版或企业版方案${match[1]||''}`;
}
function translateConnectedTeamFragments(node){
  if(!node||node.nodeType!==Node.TEXT_NODE)return false;
  const fragments=isConnectedTeamContext(node.parentElement)?connectedTeamFragments:isFolderTransferContext(node.parentElement)?folderTransferFragments:undefined;
  if(!fragments)return false;
  let translated=node.nodeValue;
  for(const [source,target] of fragments){
    if(translated.includes(source))translated=translated.split(source).join(target);
  }
  if(translated===node.nodeValue)return false;
  applyTranslatedText(node,node.nodeValue,translated);
  return true;
}
function shouldTranslateAttribute(element,name){
  return name!=='data-tooltip'||element?.getAttribute?.('data-tooltip-type')!=='lookup';
}
function translateAttributes(element){
  for(const name of attrs){
    if(!shouldTranslateAttribute(element,name))continue;
    const value=element.getAttribute(name);
    const translated=value&&lookup(value,element);
    if(translated&&translated!==value)element.setAttribute(name,translated);
  }
}
function ensureControlledAutoOverlayStyle(){
  if(document.getElementById(controlledAutoOverlayStyleId))return;
  const style=document.createElement('style');
  style.id=controlledAutoOverlayStyleId;
  style.textContent=`[${controlledAutoOverlayAttribute}]{position:relative!important}[${controlledAutoOverlayAttribute}]>input{color:transparent!important;-webkit-text-fill-color:transparent!important;caret-color:transparent!important}[${controlledAutoOverlayAttribute}]::after{content:'自动';position:absolute;inset:0;display:flex;align-items:center;color:inherit;font:inherit;pointer-events:none}`;
  (document.head||document.documentElement).appendChild(style);
}
function translateControlledAutoValue(element){
  if(element?.tagName!=='INPUT')return;
  const layoutGuideCount=!!element.closest?.('[class*="grid_settings--stripesNumSections--"]');
  if(!layoutGuideCount&&!isTypographyLineHeightAutoInput(element))return;
  const host=element.parentElement;
  if(!host)return;
  const sync=()=>{
    if(!element.isConnected)return;
    const showOverlay=element.value==='Auto'||element.value==='自动';
    if(showOverlay)ensureControlledAutoOverlayStyle();
    host.toggleAttribute(controlledAutoOverlayAttribute,showOverlay);
  };
  if(!controlledAutoInputs.has(element)){
    controlledAutoInputs.add(element);
    const refresh=()=>{
      queueMicrotask(sync);
      requestAnimationFrame(()=>requestAnimationFrame(sync));
    };
    for(const type of ['focus','click','input','change','blur'])element.addEventListener(type,refresh);
  }
  sync();
}
function translateReadonlyMixedValue(element){
  if(!element||!('value' in element)||element.value!=='Mixed')return;
  const editable=element.isContentEditable||element.matches?.('[contenteditable="true"]');
  if(editable||isComponentUserNameContext(element))return;
  element.value='多个值';
  if(element.getAttribute?.('value')==='Mixed')element.setAttribute('value','多个值');
  if(element.getAttribute?.('aria-valuetext')==='Mixed')element.setAttribute('aria-valuetext','多个值');
}
let mixedInputDisplayInstalled=false;
function installMixedInputDisplay(){
  if(mixedInputDisplayInstalled)return;
  let installed=false;
  for(const prototype of [globalThis.HTMLInputElement?.prototype,globalThis.HTMLTextAreaElement?.prototype]){
    const descriptor=prototype&&Object.getOwnPropertyDescriptor(prototype,'value');
    if(!descriptor?.configurable||!descriptor.get||!descriptor.set)continue;
    const get=descriptor.get,set=descriptor.set;
    Object.defineProperty(prototype,'value',{configurable:true,enumerable:descriptor.enumerable,get(){const value=get.call(this);if(value==='Mixed'&&!this.isContentEditable&&!isComponentUserNameContext(this))return '多个值';return value;},set(value){set.call(this,value==='Mixed'&&!this.isContentEditable&&!isComponentUserNameContext(this)?'多个值':value);}});
    installed=true;
  }
  if(!installed)return;
  mixedInputDisplayInstalled=true;
}
function translateMixedTextNode(node){
  if(!node||node.nodeType!==Node.TEXT_NODE||node.nodeValue.trim()!=='Mixed')return false;
  const element=node.parentElement;
  if(!element||isComponentUserNameContext(element)||isCodeSyntaxContext(element))return false;
  const value=node.nodeValue;
  const leading=value.match(/^\s*/)?.[0]??'',trailing=value.match(/\s*$/)?.[0]??'';
  applyTranslatedText(node,value,`${leading}多个值${trailing}`);
  return true;
}
function translateTextNode(node){
  if(translateConnectedTeamFragments(node))return;
  if(translateMixedTextNode(node))return;
  const value=node.nodeValue;
  const translated=lookup(value,node);
  if(translated&&translated!==value)applyTranslatedText(node,value,translated);
}
function translateNode(node){
  if(!node)return;
  if(node.nodeType===Node.TEXT_NODE){translateTextNode(node);compactMotionBetaTooltip(node);return;}
  if(node.nodeType!==Node.ELEMENT_NODE&&node.nodeType!==Node.DOCUMENT_FRAGMENT_NODE)return;
  if(node.nodeType===Node.ELEMENT_NODE){translateFolderTransferPlanElement(node);translateControlledAutoValue(node);translateReadonlyMixedValue(node);translateAttributes(node);}
  const walker=document.createTreeWalker(node,NodeFilter.SHOW_ELEMENT|NodeFilter.SHOW_TEXT);
  let current;while(current=walker.nextNode()){
    if(current.nodeType===Node.TEXT_NODE)translateTextNode(current);
    else{translateFolderTransferPlanElement(current);translateControlledAutoValue(current);translateReadonlyMixedValue(current);translateAttributes(current);}
  }
  compactMotionBetaTooltip(node);
}
function collectMutationRoots(records){
  const roots=new Set();
  for(const record of records){
    if(record.type==='characterData'||record.type==='attributes')roots.add(record.target);
    else for(const node of record.addedNodes)roots.add(node);
  }
  const result=[];
  rootLoop:for(const root of roots){
    for(let parent=root.parentNode;parent;parent=parent.parentNode){if(roots.has(parent))continue rootLoop;}
    result.push(root);
  }
  return result;
}
function start(){
  if(!document.body)return;
  installMixedInputDisplay();
  const observer=new MutationObserver(records=>{for(const root of collectMutationRoots(records))translateNode(root);});
  observer.observe(document.body,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:attrs});
  translateNode(document.body);
  const pending=observer.takeRecords();
  for(const root of collectMutationRoots(pending))translateNode(root);
  window.__FigmaZhCN={version:'2026-07-17',entries:Object.keys(map).length,translate:translateNode};
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();
