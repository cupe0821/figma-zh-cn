...
这是一个面向 Figma 桌面版的非官方简体中文翻译与本地维护项目。当前基线为 **Figma 126.7.8 / 中文包 126.7.8-cn.1**，包含 4610 条累计词典、着色器与 AI 界面动态翻译、原生菜单翻译和轻量动态界面运行时。
...
| 平台 | 发布包 | 当前状态 |
| --- | --- | --- |
| macOS Intel | `macos-x64` | 已在 126.7.8 实机验证 |
| macOS Apple Silicon | `macos-arm64` | 同源安装流程，待 126.7.8 实机复验 |
...
4. 安装器会备份原始 `app.asar`、写入中文加载器及原生菜单翻译调用、同步更新 Figma 的 ASAR 尾部大小标记、重新签名并启动 Figma。

macOS 首次从官方签名或 `cn.4` 及更早版本迁移到 `cn.5` 时，可能显示“找不到用于储存 Figma Key 的钥匙串”。这是修改官方应用后切换到稳定本地签名的首次迁移提示。选择“还原为默认”后可能需要重新登录 Figma；安装器不会静默删除或读取钥匙串内容。`cn.5` 起固定使用 `com.figma.Desktop` 指定要求，后续中文包升级不再随文件内容生成新的临时身份。
...
- `manifests/source-lock.json` 自动锁定加载器目录中的全部源文件及 SHA-256；
...
- `lang.cn.json` 保存累计界面词典，更新时只追加或修正经过截图确认的字符串；
- `lang.cn.menu.json` 保存原生菜单和桌面字符串。为保持既有覆盖率，渲染器仍兼容其中的历史词条，菜单翻译则始终以该文件为最终优先级；
- `figmaCN.js` 只处理动态句式、专业名词保护、组件用户命名保护和着色器等必须判断上下文的内容；
- 每次页面导航只向主 frame 注入一次；子 frame 加载时只向触发事件的目标 frame 注入；
- DOM 更新会合并同批次中互相包含的节点，再执行翻译，避免重复遍历同一子树。

不要把组件名称、变量或 mode 名称、代码/CSS、字体名称、色彩空间名称加入无上下文的全局替换。确需翻译同名系统文本时，应在 `figmaCN.js` 增加可验证的上下文规则，并补充回归测试。
...
可通过 `--resources /path/to/Resources` 对测试副本执行操作。补丁器只接受具有已知 `"use strict"` 启动锚点和唯一菜单构建锚点的入口；安装后必须准确检测到 3 处菜单调用及 1 处桌面字符串调用，否则立即回滚。
